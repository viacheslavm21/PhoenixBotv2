...
class CameraOpt():
    def __init__(self, env_name, pred_filename_k, pred_filename_b, initial_calib_model, save_or_read='save'):
        ...
        self.R_cam2gripper = initial_calib_model.R_cam2gripper
        self.t_cam2gripper = initial_calib_model.t_cam2gripper
        self.mtx = initial_calib_model.intrs
        self.dist = initial_calib_model.dist
        ...
        for path, dirs, files in os.walk(f"{env_name}/raw"):
            for file in files:
                if file[-3:] == 'jpg':
        ...
        if save_or_read == 'read':
            ...
            with open(pred_filename_k, 'rb') as f:
                self.keypoint_prediction_dict = pickle.load(f)
            with open(pred_filename_b, 'rb') as f:
        ...
        if save_or_read == 'save':
            ...
            with open(pred_filename_k, 'wb') as f:
                pickle.dump(self.keypoint_prediction_dict, f)
            with open(pred_filename_b, 'wb') as f:
    ...
    def reproject_and_draw(self, points_3d, calib_model: CalibrationModel):
        ...
        frame_points_list = []
        frame_bboxes_list = []
        keys = multiplicate_and_concatenate(list(self.img_fns_dict.keys()), int(25 / 4))
        combination = multiplicate_and_concatenate(list(range(25)), int(4))
    ...
    def optimize_calib_re_least_squares(self, calib_model: CalibrationModel):
        ...
        for combination_id, combination in enumerate(combinations):
            ...
            for idx, key in zip(combination, keys):
                ...
                #print("self.keypoint_prediction_dict[key][idx]", self.keypoint_prediction_dict[key][idx])
        ...
        combination = np.concatenate((np.asarray(list(range(25))), np.asarray(list(range(25))), np.asarray(list(range(25))), np.asarray(list(range(25)))))
        for idx, key in zip(combination, keys):
    ...
    def loss_positioning(self, params):
        # print('entered')
        self.experiment.R_cam2gripper = R.from_rotvec(params[:3]).as_matrix()
        self.experiment.t_cam2gripper = [[params[3]], [params[4]], [params[5]]]
        # print(self.R_cam2gripper, self.t_cam2gripper)
        valid_mean_errors = self.experiment.run_experiment(n_runs=1000)
        mean_error_over_keypoints = np.mean(valid_mean_errors, axis=0)
        loss = sum(mean_error_over_keypoints)
        print(loss)
        return loss

    def optimize_calib_pos_genetic(self, calib_model: CalibrationModel, experiment):

        self.experiment = experiment

        R_cam2gripper, t_cam2gripper, mtx = R.from_matrix(calib_model.R_cam2gripper).as_rotvec(), \
                                            np.asarray(calib_model.t_cam2gripper).T[0], calib_model.intrs
        ...
        print("res_de", res_de.x)
        return res_de.x.reshape(-1, 3)
